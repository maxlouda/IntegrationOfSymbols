...
\textbf{Question}: How do we handle constant polynomials? If the polynomial is equal to one or zero, the tensor product vanishes. But for all other rational numbers the tensor product should not vanish and is unrelated to each other. For example $2 \otimes (1+x)$ is linearly independent to $3 \otimes (1+x) = (2 \cdot 3/2) \otimes (1+x) = 2 \otimes (1+x) + (3/2) \otimes (1+x)$. Why is only $2 \in \overline{P_S}$ but not $3 \in \overline{P_S}$? Exception: Powers of constant polynomials already considered need not be included, of course.
...
\textbf{Question}: Clearly, this group action can be used to traverse $\rff_S^{(1)}$ in a more efficient manner. But how exactly? I'd propose the following procedure.
\begin{itemize}
...
The $(3,2,1)$-shuffle is given by
\begin{equation*}
(a_1 \otimes a_2 \otimes a_3) \shuffle (a_4 \otimes a_5) \shuffle (a_6).
\end{equation*}
