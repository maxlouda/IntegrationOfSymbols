...
FindPrimes::usage = "In: list of multiplicatively independent polynomials with n variables. Out: list of n not necessarily different prime numbers.";
...
FindPrimes[PIlist_] :=
    Block[{res, vars, primes, functions, found, n, pos},
        ...
        While[
            found == False
            ,
            primes = Tuples[Prime /@ Range[n], Length[vars]];
            res = Outer[#2 @@ #1&, primes, functions, 1];
            found = SelectFirst[res, Length[DeleteDuplicates[#]] == Length[
                functions]&,False];
